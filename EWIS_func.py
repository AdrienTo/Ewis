...
class CalcMassFunc(ot.OpenTURNSPythonFunction):
        ...
        def __init__(self, x1, x2, x3):
            super(CalcMassFunc,self).__init__(len(x2)*x1[0],2)
            self.x1 = x1             # x1[0]: Total nb of equipment requesting power, x1[1]: Constant deltaV*V0
            self.x2 = x2             # Vector of distances b/t EPDC & components of the EWIS architecture
        ...
        def _exec(self, X):
            ...
            Mass = np.sum(np.array(self.x2).ravel() * self.x3[indexes, 1])
            # Compute the diameter of the bundle corresponding to the sum of all diameters of elementary bundles
            Diameter = np.sum(np.array(self.x3[indexes, 2]**2))
